...
namespace StudyGroup_LDH._01.Basic
{
    internal class GarbageCollector
    {
        ...
        public static void Main(string[] args)
        {
            ...
            // 메모리를 특정 크기 블록들로 나눠놓고 적합한 곳을 찾아가는 작업 진행한다.
            ...
            // (2) 그래서 단점은?
            // - 높은 세대 자원들을 반납하고 싶은데, 하위 세대가 가득 차지 않는 이상 높은 세대 메모리를 수집할 생각을 하지 않는다.

            // 2-1. 세대 확인하기
            Console.WriteLine(GC.GetGeneration(a1)); // 0세대
            Console.WriteLine(GC.GetGeneration(b1));
            Console.WriteLine(GC.GetGeneration(c1));

            GC.Collect(0); // 0세대에 대해 GC를 해달라는 의미이다.
            Console.WriteLine("-----------------");
            ...
            // 괜히 세대만 올라가는 경우가 생기고, 반납까지 시간이 더 오래걸리므로, GC 자체에서 알아서 관리하도록 하자.
        }
    }
}
