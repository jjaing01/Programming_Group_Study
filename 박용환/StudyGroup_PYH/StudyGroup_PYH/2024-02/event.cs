...
namespace StudyGroup_PYH._2024_02
{
    ...
    class Button
    {
        ...
        public void Press()
        {
            // 값의 존재 여부를 확인할 수 없다.
            handler_test();
            handler_event?.Invoke(); // elvis operator
        }
    ...
    class @event
    {
        public static void main()
        {
            ...
            btn.handler_test = F3; // C라는 사람이 실수로 F3 대입을 해버렸음 => 앞에 사용자들이 등록했던 기록들이 사라진다.
            btn.Press();

            // 위 실수를 방지하기 위해 event를 사용한다.
            btn.handler_event += F1; // A라는 사람이 등록했음
            btn.handler_event += F2; // B라는 사람이 등록했음
            btn.handler_event += F3; // C라는 사람이 실수로 F3 대입을 해버렸음 => 앞에 사용자들이 등록했던 기록들이 사라진다.
            btn.Press();
        }
