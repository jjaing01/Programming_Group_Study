...
namespace StudyGroup_PYH._2024_04
{
    internal class GarbageCollector
    {
        ...
        {
            // 1. C# healp 메모리 할당은 연속적인 공간에 발생한다.
            // (1) C/C++ heap 메모리 할당할 경우
            // 메모리를 특정 크기 블록들로 나눠놓고 적합한 곳을 찾아가는 작업 진행
            ...
            // (2) 그래서 단점은?
            // - 높은 세대 자원들을 반납하고 싶은데, 하위 세대가 가득차지 않는 이상 높은 세대 메모리를 수집할 생각을 하지 않는다.

            // 2-1 세대 확인하기
            Console.WriteLine(GC.GetGeneration(a1)); // 0세대
            GC.Collect(0); // 0 세대에 대해 GC를 해달라는 의미
